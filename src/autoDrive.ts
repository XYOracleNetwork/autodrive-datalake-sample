...
import type { SampleStorage } from './contracts.js'
...
function operationOwner(archivist: S3Archivist, callerSignal?: AbortSignal) {
  ...
  const run = async <T>(operation: () => Promise<T>, message: string): Promise<T> => {
    requireCondition(!signal.aborted && closing === undefined, 'auto-drive-closed', 'Auto Drive storage is stopped; inspect the retained evidence before retrying', 503)
    let interrupted: (() => void) | undefined
    const timer = setTimeout(() => controller.abort(), OPERATION_TIMEOUT_MS)
    const result = Promise.race([
      operation(),
      new Promise<never>((_resolve, reject) => {
        interrupted = () => reject(new SampleError('auto-drive-aborted', 'Auto Drive operation was cancelled or timed out; inspect the retained evidence before retrying', 503))
 ...
 * The CLI should enable AbstractModule.enableLazyLoad before constructing SDK modules,
