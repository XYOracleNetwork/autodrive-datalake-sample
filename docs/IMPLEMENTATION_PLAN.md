...
Status: planned; repository foundation only. No sample website, wallet
transaction, Auto Drive upload, or deployment has been executed by this
repository. Research baseline: September 9, 2026.
...
| Admitted schema | `network.xyo.autodrive.sample.payload.v1` only |
| Payload form | `{ "schema": "network.xyo.autodrive.sample.payload.v1", "data": { ... } }` |
...
| Durable payload backend | The existing Aries Auto Drive data plane, using XYO SDK 7.4.1 or a subsequently qualified release |
...
| Provider credentials | Only the Auto Drive data-plane process receives `AUTODRIVE_API_KEY` |
...
autodrive-datalake-sample/
  ...
  test/            # future explicit live and wallet acceptance harnesses
  deploy/          # future local service composition and deployment instructions
```

The initialization milestone reserves these workspaces. Application manifests,
source, and dependencies are added when their implementation begins; no dummy
Perma-Store button or passing mock demonstration should be shipped as a working
feature.
...
Provide a JSON object editor for `data`, a read-only full payload preview, and a
live **bytes / 4,096** counter. The schema is fixed. Use simple formatted JSON;
...
2. Build the final `{ schema, data }` object and serialize it with dapp-kit's
...
sample Node service can use that public client to reach the separately deployed
plane. [Sources: plane manifest][plane-package], [plane implementation][plane],
[HTTP client][client].

The local deployment milestone builds the official plane from its pinned source
using its existing root-context Dockerfile, or consumes a subsequently published
and verified image digest. Confirm a clean build and runtime before documenting
an image as available. The inspected Dockerfile uses Node 22 while this sample
standardizes on Node 24; qualify that service independently and fix an upstream
container incompatibility if one appears. Do not assume source tests establish
container readiness. [Source: plane Dockerfile][plane-docker].
...
Data-plane process:
  ...
  AUTODRIVE_ALLOWED_SCHEMAS=network.xyo.autodrive.sample.payload.v1
...
Datalake descriptor:
  ...
  allowedSchemas: [network.xyo.autodrive.sample.payload.v1]
...
The service identity receives only append/read/usage operations needed for the
sample, with no public append, delete, clear, or administration route.
...
`http://127.0.0.1:5173` locally with the Vite API proxy, and HTTPS when deployed.
...
These are **new sample endpoints to implement**, not existing Aries routes:
...
outcomes. The local composition should supply PostgreSQL for atomic reservation
and uniqueness constraints; retain it across service restarts. It can share the
Aries control-plane PostgreSQL process using separate schemas/roles. A browser refresh
...
The main page contains network and wallet status, the JSON data editor, normalized
...
- Build/run the pinned official Aries plane and persistent control store locally.
  Verify its schema policy, service credential scope, and read path with an
  ordinary test store before enabling the real provider.
...
Exit: package and service contracts are reproducible, network choice is explicit,
and no unsupported wallet override is required.
...
The normal root test suite runs offline. Add meaningful protocol, profile,
service, and orchestration tests as implementation arrives; initialization may
explicitly allow no tests. A mocked store is valid fault-injection evidence, not
proof that Auto Drive accepted or retained anything.
...
pnpm test                  # ordinary offline suite
pnpm test:autodrive         # future opt-in real storage suite, excluded above
pnpm test:wallet:chrome     # future real extension acceptance harness/runbook
```

The last two commands are planned and must not be represented as available until
implemented. CI must not receive `AUTODRIVE_API_KEY`, wallet secrets, or funded
chain signing capabilities. A key appearing in an environment must never activate
live tests implicitly. Use explicit test configuration, excluded live directories,
and an opt-in marker in the live test itself.
...
| dapp-kit | `@xyo-network/dapp-kit@2.0.0` | Published; actual profile used by this sample still to implement/test |
...
container behavior, measured storage latency versus transaction expiry, and
finalized end-to-end operation remain explicit implementation gates.
